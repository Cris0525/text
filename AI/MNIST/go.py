...
def cut(img):
    ...
    for cnt in contours:
        ...
        if x != 0 and y != 0 and w*h >= 500:
            ...
            member = []
            flag += 1
    lists.sort(key=takeSecond)
    return lists


def imageprepare(lists):
    ...
    for result in result_list:
        ...
        #predint2=prediction.eval(feed_dict={x: [result],keep_prob: 1.0}, session=sess)
        #print('recognize result:')
        #print(predint[0])
        res = res + bytes(predint[0])
    return res
...
def process_img(filedir):
    ...
    lists = cut(thresh)
    result = imageprepare(lists)
    return result


if __name__ == "__main__":
    print process_img('image/timg.jpeg')
