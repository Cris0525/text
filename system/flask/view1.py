...
from spider import spider_login, getgrades, parser,get_timetable,parser_timetable
...
def score():
	if request.method=='GET':
	    ...
	    credit.append('all')
	    credit.append('all')
	    return render_templates("score.html",classes=stu.subject,credit=credit)
	else:
	    id = session.get('id')
...
def timetable():
	...
	term = request.form.get('term')
	year = request.form.get('year')
	time_table = get_timetable(id,name,term,year)
	parser_timetable(time_table)
	return render_template('time_table.html',year=year,term=term)
