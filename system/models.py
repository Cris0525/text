...
stu_with_sub = db.Table('stu_with_sub',
                        db.Column('student_id', db.BIGINT, db.ForeignKey('student_information.id'),primary_key=True),
                        db.Column('subject_id', db.Integer, db.ForeignKey('subject_information.id'),primary_key=True),
                        )
...
class Student(db.Model):
    ...
    subject = db.relationship('Subject', secondary = stu_with_sub, backref = db.backref('student'))

    def __init__(self, id, password, name, score_path=None):
        self.id = id
        self.name = name
...
class Score(db.Model):
    ...
    GPA = db.Column(db.Integer, nullable=False)                                                  #绩点
