...
from spider import spider_login, getgrades, parser, getxskbcx, parsers
...
def my_before_request():
    ...
    if id:
        if user == 'student':
            ...
            g.user = Student.query.filter(User.id == id).first()
...
def kbcx():
    name = session.get('name')
    id = session.get('id')
    xskb = getxskbcx(id,name)
    parsers(id,xskb)
    return render_template("/xskb"+str(id)+".html")
...
def studentlogin():
    if request.method == 'GET':
        ...
        user = Student.query.filter(User.id == id).first()
        if user:
            ...
            user = Student(id, password, username, score_path)
            db.session.add(user)
            db.session.commit()
            session['id'] = id
